...
- **[Slang](https://github.com/shader-slang/slang)** (shader-slang) — two distinct roles, and the distinction is load-bearing. **As a compiler inside the pipeline: no, permanently** — Slang never replaces DXC for HLSL (measured: two DXC flags don't forward through Slang's API, making byte-identity unprovable; the pipeline uses faithful DXC everywhere; an early in-browser spike that used Slang as a substitute frontend is dead, sample-only reference). **As an input language: yes** — ShadowDusk accepts `.slang` source and routes it `slangc → HLSL → the unchanged pipeline`, exactly as the ShaderToy frontend sits upstream of it. Entry points are marked with Slang's own `[shader("vertex")]` / `[shader("fragment")]` attributes and the technique block is synthesized (Slang has no technique/pass concept). Requires the pinned Slang toolchain (`pwsh tools/setup-local-testing.ps1 -WithSlang`, or `SHADOWDUSK_SLANGC`) — it does not yet ship inside the NuGet packages. No route through Slang is `mgfxc`-equivalent; `mgfxc` cannot read Slang at all.
