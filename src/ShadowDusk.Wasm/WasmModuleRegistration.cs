...
/// <summary>
/// Self-registers the two faithful <c>[JSImport]</c> modules ShadowDusk.Wasm ships as
/// Blazor static web assets — <c>shadowdusk-dxc</c> (faithful DXC→WASM) and
/// <c>shadowdusk-spirv-cross</c> (SPIRV-Cross→WASM) — so a consumer that adds ONLY a
...
internal static class WasmModuleRegistration
{
    ...
    /// <summary>
    /// Idempotently imports (registers) both <c>[JSImport]</c> modules from the
    ...
    /// <c>[JSImport]</c> call in either backend is what makes the consumer's experience
    /// zero-wiring. The heavy <c>dxcompiler.wasm</c> (~17.4 MB) is NOT downloaded here —
    /// registering the <c>shadowdusk-dxc</c> shim only evaluates its tiny JS; the shim
    /// lazy-loads the DXC WASM on the first <c>compileToSpirv</c> via
    /// <c>DxcInterop.EnsureReadyAsync</c>.
    /// </summary>
