...
single-pass **ShaderToy** GLSL image shader into a self-contained HLSL **`.fx`** source file. Once
you have the `.fx`, the **existing ShadowDusk pipeline** compiles it to MonoGame/KNI (OpenGL/DirectX)
and, within fx_2_0/SM3 limits, FNA — with zero pipeline changes and no native dependency.
...
Single-pass image shaders only (one `mainImage`, optional Common tab). No multipass (Buffer A–D),
no audio/video/cubemap channels. Unsupported constructs **fail loudly** (clear diagnostic + non-zero
