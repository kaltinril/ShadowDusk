...
/// </summary>
internal readonly record struct GlslType(ScalarKind Scalar, int Rows, int Cols, string? StructName = null)
{
    ...
    /// <summary>A user-defined struct value of the named type (G6).</summary>
    public static GlslType Struct(string name) => new(ScalarKind.Struct, 1, 1, name);
    ...
    public bool IsVector => Cols == 1 && Rows >= 2 && Scalar != ScalarKind.Struct;

    /// <summary>True for a 1-component scalar (not a struct value).</summary>
    public bool IsScalar => Rows == 1 && Cols == 1 && Scalar != ScalarKind.Struct;

    /// <summary>True for a user-defined struct value (G6).</summary>
    public bool IsStruct => Scalar == ScalarKind.Struct;
