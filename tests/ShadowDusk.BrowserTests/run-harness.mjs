...
const PUBLISH_ROOT = path.join(__dirname, '.publish', 'wwwroot');
...
async function main() {
  ...
  await fs.access(path.join(PUBLISH_ROOT, 'index.html')).catch(() => {
    throw new Error(`Publish not found at ${PUBLISH_ROOT}. Run: node publish-sample.mjs`);
  });
...
async function writeResults(results) {
  const lines = [];
  lines.push('# Phase 24 — Browser Render Validation results');
  lines.push('');
  lines.push(`_Generated by \`run-harness.mjs\` — headless Chromium (ANGLE/SwiftShader), KNI WebGL, ${SIZE}x${SIZE}._`);
  lines.push('');
  lines.push('## Mode 1 — precompiled `.mgfx` load + render in real KNI WebGL `Effect`');
  lines.push('');
  lines.push('Reference = the SAME `.mgfx` bytes rendered on desktop **DesktopGL** (`RefRenderer`), so any diff isolates the WebGL-vs-DesktopGL question (Phase 24 risk #2). Tolerance policy: Phase 17 §6.1 (start exact; any tolerance>0 listed with observed max delta + reason).');
  lines.push('');
  ...
  lines.push('- `references/*.png` — desktop DesktopGL renders of the same bytes (RefRenderer).');
  lines.push('- `captures/*.png` — headless KNI WebGL canvas readbacks.');
  lines.push('- `diffs/*_diff.png` — magenta where over tolerance, reference elsewhere (§6.2).');
  lines.push('');
  lines.push('## Handoff');
  lines.push('Harness is headless + self-contained for **Phase 30 §16** (cross-platform CI): `node publish-sample.mjs && node run-harness.mjs` after `npm ci` + `npx playwright install --with-deps chromium`. See `README.md`.');

  await fs.writeFile(path.join(__dirname, 'RESULTS.md'), lines.join('\n') + '\n');
  console.log(`[harness] wrote RESULTS.md`);
}
