...
// The KNI runtime version under test, read from the sample's pin so every RESULTS file
// self-documents which KNI line actually rendered (the Phase-24 run was ambiguous on
// this — "pre-v4.02" could only be inferred, not read — which is why Phase 44 D had to
// re-run it). The pin is the source of truth; the resolved build is in the published
// _framework. Best-effort: never let a stamp-read failure break the harness.
const KNI_VERSION = (() => {
  try {
    const csproj = readFileSync(
      path.join(__dirname, '..', '..', 'samples', 'ShaderFiddle.Web', 'ShaderFiddle.Web.csproj'),
      'utf8');
    const m = csproj.match(/nkast\.Kni\.Platform\.Blazor\.GL"\s+Version="([^"]+)"/);
    return m ? `nkast.Kni.Platform.Blazor.GL ${m[1]}` : 'unknown (pin not found)';
  } catch {
    return 'unknown (csproj unreadable)';
  }
})();
...
async function writeHidefResults(results) {
  ...
  if (isRed) {
    ...
    lines.push(`_KNI runtime under test (pin): ${KNI_VERSION}._`);
    lines.push('');
