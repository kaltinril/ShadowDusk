...
/// <summary>
/// The Windows-only <c>D3DReflect</c> (<c>ID3D11ShaderReflection</c>) reflection path,
/// preserved verbatim as a TEST ORACLE after Phase 18 Track A replaced it in the product
/// with the pure-managed <c>RdefReader</c>. This is the exact code
/// <c>DxbcReflectionExtractor</c> shipped with through Phase 37 (minus the off-Windows
/// SD0210 guard); <c>DxbcReflectionParityTests</c> asserts the managed reader's
/// <see cref="ReflectedEffect"/> is deeply equal to this oracle's, for both
/// d3dcompiler_47 and vkd3d DXBC. Never referenced by product code.
/// </summary>
internal static class D3DReflectOracle
{
    [SupportedOSPlatform("windows")]
    public static Result<ReflectedEffect, ShaderError> Extract(ReadOnlyMemory<byte> dxbcBlob)
    {
        try
        {
