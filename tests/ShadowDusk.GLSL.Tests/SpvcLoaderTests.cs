...
{
    [Theory]
    // Windows wins regardless of arch (the shipped native is win-x64 only).
    [InlineData(true, false, false, Architecture.X64, "win-x64")]
    [InlineData(true, false, false, Architecture.Arm64, "win-x64")]
    ...
    // Linux is the default fallthrough.
    [InlineData(false, false, false, Architecture.X64, "linux-x64")]
    [InlineData(false, false, false, Architecture.Arm64, "linux-x64")]
