...
public sealed class Phase45PreParserRobustnessCorpusTests
{
    ...
    public async Task Phase45Fixture_OpenGL_CompilesToValidMgfx(string fx)
    {
        ...
        result.ExitCode.ShouldBe(0, customMessage: $"'{fx}' (Phase 45 regression) must compile for OpenGL; stderr: {result.Stderr}");
    ...
    public async Task Phase45Fixture_DirectX11_CompilesToValidMgfx(string fx)
    {
        ...
        result.ExitCode.ShouldBe(0, customMessage: $"'{fx}' (Phase 45 regression) must compile for DirectX_11; stderr: {result.Stderr}");
        result.Mgfx.ShouldNotBeEmpty("a successful compile must emit output bytes");
    ...
    public async Task Phase45Fixture_Fna_CompilesToValidFx2Binary(string fx)
    {
        ...
        result.IsSuccess.ShouldBeTrue($"'{fx}' (Phase 45 regression) is in the SM <= 3 FNA subset and must compile; " +
