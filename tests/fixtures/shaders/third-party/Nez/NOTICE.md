...
limitation (noted), not a ShadowDusk parser defect, except `Noise.fx` (see note).
...
| `Noise.fx` | MIT | DX + FNA | GL: a **ShadowDusk bug** — a uniform literally named `noise` is renamed to `_noise` by SPIRV-Cross (GLSL reserved word) but the reflected parameter list still says `noise`, so the GL cbuffer/parameter join fails with `SD0012`. `noise` is valid HLSL that fxc/mgfxc accept. Tracked as a Phase-45 catalogue candidate (see `docs/test-shader-corpus.md`); vendored and wired on DX + FNA only until fixed. |
