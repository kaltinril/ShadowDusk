...
//   Project    : Apos.Shapes (Apostolique/Apos.Shapes)
//   Repo       : https://github.com/Apostolique/Apos.Shapes
//   Commit     : ea38c6d8ee67f774cabb9e2952c195a9b9c2d330
//   Upstream   : Source/Content/apos-shapes.fx
//   License    : MIT - Copyright (c) 2021 Jean-David Moisan (see ./LICENSE, ./NOTICE.md)
//   Exercises  : THE ISSUE #145 REPRODUCER, verbatim. The current upstream revision of
//                Gum's SDF shape renderer, with the `#elif SM6` profile branch the
//                Vulkan target selects: vs_6_0/ps_6_0, THREE Texture2D/SamplerState
//                pairs at explicit registers (t0/s0, t1/s1, t2/s2), wrapper functions
//                around .Sample, a 13-element vertex input (POSITION0, TEXCOORD0-9,
//                POSITION1, NORMAL0), a float4x4 view_projection in $Globals, the
//                FixSnorm workaround for MonoGame's SSCALED vertex-format bug, plus
//                ddx/ddy footprints, Newton-iteration loops, dash SDFs and Oklab
//                gradient math. This is the shader whose SM6 build rendered NOTHING
//                (the row-major matrix bug) and whose legacy build access-violated.
//
//                Kept ALONGSIDE apos-shapes.fx (commit 3fb73b8, the legacy
//                sampler/tex2D revision) and apos-shapes-aa.fx (commit d507a734, the
//                issue #136 derivative pin) rather than replacing them: each pins a
//                distinct, still-relevant regression shape.
// =============================================================================
#if __KNIFX__
