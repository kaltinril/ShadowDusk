...
|---|---|---|---|
| `apos-shapes.fx` | MIT | **GL + DX + FNA** | **FNA (since vkd3d 2.1):** the shader has no SM3 / FNA profile branch — its `#if OPENGL` arm selects `ps_3_0`, the `#else` selects `ps_4_0` (SM4), and the FNA target takes the SM4 arm, which ShadowDusk's macro-profile policy defaults to the SM3 ceiling. vkd3d 1.17 then rejected the `EllipseSDF` Newton-iteration `for` loop, whose trip count is a runtime `int`, with `E5017: Aborting due to not yet implemented feature: Instruction type HLSL_IR_LOOP`. vkd3d 2.0 implemented SM2-3 loops, so it compiles — that was a compiler gap, never a shader-model limit. The reporter's **current** upstream copy (3235 lines) lives at `tests/fixtures/issues/202/` with its own NOTICE, outside the corpus sweeps; it still exceeds the SM3 temporary file (`E9015`). |
