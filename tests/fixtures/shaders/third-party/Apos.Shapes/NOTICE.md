...
|---|---|---|---|
| `apos-shapes.fx` | MIT | **GL + DX** | **FNA:** the shader has no SM3 / FNA profile branch — its `#if OPENGL` arm selects `ps_3_0`, the `#else` selects `ps_4_0` (SM4), and the FNA target takes the SM4 arm. ShadowDusk's macro-profile policy defaults that to the SM3 ceiling, and vkd3d 1.17 then rejects the `EllipseSDF` Newton-iteration `for` loop, whose trip count is a runtime `int`, with `E5017: Aborting due to not yet implemented feature: Instruction type HLSL_IR_LOOP` — reported on the `for` at line 204 (the location is pinned by `FnaDiagnosticLocationTests`; before issue #202 vkd3d's own drifted coordinates were surfaced). This is consistent with Apos.Shapes shipping for MonoGame OpenGL/DX, not FNA — a legitimate SM limit, not a ShadowDusk defect. The reporter's **current** upstream copy (3235 lines) lives at `tests/fixtures/issues/202/` with its own NOTICE, outside the corpus sweeps. |
