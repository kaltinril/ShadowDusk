...
- **Slang** — a shading language from [shader-slang](https://github.com/shader-slang/slang). ShadowDusk accepts `.slang` source as an input format (alongside `.fx`) in two tiers. The free default, built into `ShadowDusk.Compiler`, accepts the HLSL-compatible subset: a pure managed text transform converts it to `.fx` text before the normal pipeline compiles it, with no Slang binary shipped or run anywhere. The optional `ShadowDusk.Slang` package accepts genuine Slang — `import`, generics, `interface` conformances — by bundling the real `slangc` compiler and routing its HLSL emission to the same unchanged pipeline.
- **slangc** — the real Slang compiler's own command-line tool. Two distinct roles: as a **test-time oracle** for the free subset frontend above (like `fxc`/`mgfxc` elsewhere), never shipped or invoked by the product, cross-validating that ShadowDusk's Slang corpus is genuine Slang and that ShadowDusk's reading of it agrees with slangc's own HLSL emission; and as the **real compiler the optional `ShadowDusk.Slang` package bundles and invokes** (win-x64 today) to compile genuine Slang source for a consumer who opts in.
