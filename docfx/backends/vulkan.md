...
One caveat: a byte-for-byte pixel comparison against MonoGame's own `mgfxc /Profile:Vulkan` output isn't currently possible. `mgfxc`'s compiled Vulkan output crashes on load in real DesktopVK for shaders using auto-numbered (non-explicit-register) resources — a MonoGame-side bug, independent of ShadowDusk, unrelated to anything this library emits. ShadowDusk's own output sidesteps it by construction. Track MonoGame's `DesktopVK` runtime maturity if you rely on this target in production.
