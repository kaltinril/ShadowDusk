...
- **A few codes outside that range are warnings too**, marked as such in the table: `SD0007` (a ShaderToy conversion adjustment), `SD0008` (an `#include` that only resolves because the file system ignores case), `SD0104` (an unrecognized vertex-input semantic defaulted to TextureCoordinate, exactly as `mgfxc` defaults and warns), and `SD0214` (DirectX12 DXIL compiled unsigned on a non-Windows host). Like every warning, they never gate output.
