...
| **DirectX 12** | `PlatformTarget.DirectX12` | `DirectX_12` | `.mgfx`, **DXIL** (SM6) | ✅ Validated (MonoGame `WindowsDX12` only — KNI has no DirectX 12 platform; see [DirectX 12](../backends/directx12.md)) |

For the MonoGame/KNI targets, the on-disk **profile byte** in the MGFX header encodes the backend choice (`OpenGL = 0`, `DirectX11 = 1`, `DirectX12 = 2`, `Vulkan = 80`). The runtime reads it to pick the shader path, so the target must be chosen **at compile time** — there is no universal `.mgfx` that serves both DirectX and OpenGL. A DirectX `.mgfx` is useless to a DesktopGL game and vice versa. FNA is selected differently (it's a whole separate format, `.fxb` — see Axis 1) and is **not** byte-compatible with the `.mgfx` targets.
...
| Render-proven with a real `ContentManager` on | (n/a — `new Effect` path) | MonoGame WindowsDX + DesktopGL, KNI 4.2.9001 + 4.3.9001 (v10 and KNIFX), FNA 26.06 — pixel-identical to the reference build on each; KNI ≤ 4.2 loads ShadowDusk's `.xnb` where it rejects a stock-mgcb one |

If a consuming project loads effects with `new Effect(gd, bytes)`, hand it the raw `.mgfx` — done. If it uses `Content.Load<Effect>`, it needs an `.xnb`, and there are three ShadowDusk routes: **direct output** — name an `.xnb` output on the CLI or call `result.Value.ToXnb()`, drop the file in the content directory, and no MGCB is involved at all (see [Drop-in `mgfxc`](dropin-mgfxc.md)); for a build that already uses MGCB (a `.mgcb`) for other content, the `ShadowDusk.MgcbPlugin` content processor (see the [MGCB Content Pipeline](mgcb-content-pipeline.md) guide); or, on MonoGame 3.8.5's code-centric Content Builder project, the `ShadowDusk.ContentPipeline` library's importer/processor passed as instances (see [MonoGame 3.8.5 Content Builder](content-builder.md)). See also [Parameters & Caveats → `.mgfx` vs `.xnb`](parameters-and-caveats.md#mgfx-vs-xnb).
