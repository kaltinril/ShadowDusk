...
> The product is the in-memory `IShaderCompiler` library (the `ShadowDusk.Compiler` NuGet package): add the package and call `CompileAsync(fx)` to get the compiled effect bytes (`.mgfx`, or `.fxb` for FNA) — nothing else to install. The **CLI** (the `ShadowDuskCLI` dotnet tool), the **MGCB content-processor plugin** (`ShadowDusk.MgcbPlugin`, which a `.mgcb` `/reference:`s so MonoGame's Content Builder compiles through ShadowDusk in its own process), the **Content Builder library** (`ShadowDusk.ContentPipeline`, the same importer/processor as a library for MonoGame 3.8.5's C# Content Builder project), and **direct `.xnb` output** (`CompiledShader.ToXnb()`, or an `.xnb` output path on the CLI — `Content.Load<Effect>` ready with no MGCB and no consumer code change) are delivery shapes of the same library for build-time use, and emit the same bytes. The **in-browser shader fiddle is only a sample of reach — not a separate product.**
