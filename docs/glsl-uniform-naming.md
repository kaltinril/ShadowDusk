...
| 3 | `uniform sampler2D <id>;` | `uniform sampler2D ps_s{slot};` — one uniform per **combined (texture, sampler) pair**, `slot` coming from fxc's own allocator, *not* the pair's position in SPIRV-Cross's first-use pair list; uses renamed in the body. **The rule, in one sentence:** in **texture-declaration order**, a pair whose sampler declared an explicit register takes it, and every other pair takes the lowest register neither already taken nor **reserved**. The legacy and modern spellings are therefore *not* equivalent: at `ps_3_0` a texture and a sampler are ONE object in ONE register namespace, so a legacy `sampler X : register(sN)` **is** that combined object and lands on `N`, while a modern `SamplerState S : register(sN)` is a sampler-only object that merely **reserves** `N` — one texture plus `SamplerState S : register(s0)` yields `ps_s1`, not `ps_s0`. Verified against the pinned `mgfxc` on ten shapes (issue #189) |
