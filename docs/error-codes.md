...
| `SD0600` | Slang frontend: the `slangc` compiler was not found (or failed to start). The message lists every location probed and the provisioning command (`pwsh tools/setup-local-testing.ps1 -WithSlang`, or set `SHADOWDUSK_SLANGC`). The Slang native does not yet ship inside the NuGet packages; until it does, `.slang` input needs a provisioned toolchain. | `SlangToolchain` |
| `SD0601` | Slang frontend: `slangc` failed but its stderr could not be parsed into located diagnostics; the full raw text is carried verbatim in the message (parseable slangc diagnostics pass through as their own `E`-prefixed codes with file/line/column intact — never reworded). | `SlangToolchain` |
