...
| `SD0008` | **Warning.** An `#include` resolved only because *this* host's file system ignores case: the spelling in the directive differs from the file's real name by case alone (`#include "shared/macros.fxh"` for a file named `Shared/Macros.fxh`). Android, Linux, and case-sensitive APFS do not ignore case, so the same shader fails there with `SD0001`. Match the on-disk spelling. Only the segments the directive itself spells are checked — the absolute prefix above them is your machine's own layout and never ships. | `Preprocessor` |
...
| `SD0029` | **Warning (CLI only).** An `.xnb` output was requested with **no `/Profile:` and no `--target-runtime`**, so the container holds a `DirectX_11` (WindowsDX) effect, the CLI's mgfxc-parity default. That file is correct for a WindowsDX game, but a MonoGame DesktopGL / Android / iOS / macOS game rejects it at `Content.Load<Effect>` with *"This MGFX effect was built for a different platform!"* and KNI with *"Effect profile 'DirectX_11' is not compatible with the graphics backend 'OpenGL'"* — neither names the profile or this tool, so the warning does. Never an error and never a required flag: name the profile your game targets (e.g. `/Profile:OpenGL`) and it goes away. | `PipelineRunner` |
...
| `SD0104` | **Warning.** A vertex-input semantic is not one ShadowDusk recognizes, so its attribute defaulted to `VertexElementUsage.TextureCoordinate` with the trailing numeric index. The value is deliberately unchanged — real `mgfxc` defaults exactly the same way, and it also **prints a warning when it defaults**, so a drop-in replacement must warn rather than reject (bug-hunt 2026-07-27 N5). Matters because a typo (`TEXCORD0` for `TEXCOORD0`) silently mints a *phantom* TextureCoordinate attribute that MonoGame's `VertexInputLayout` then demands from the vertex declaration; the only symptom is a failed draw with no reference back to the shader. Raised on the Vulkan (SPIR-V) and DirectX12 (DXIL) attribute-table paths, the two targets that write this table. | `VertexSemanticMapper` via `SpirvVertexInputReflector`, `DxilVertexInputReflector` |
...
| `SD0501` | MGCB plugin: the content build's platform (`/platform:` in a `.mgcb`, `-p` in a Content Builder) names a platform ShadowDusk has no backend for (the consoles, `Stadia`, or a number with no name on the host's `TargetPlatform`). Fails the build rather than emitting a GL artifact that runtime cannot load. The message's supported list is built from `MgcbPlatformMap`, so it can never name the platform it refuses (on MonoGame 3.8.5 the old member-keyed map refused `Web` with a message listing Web as supported). | `ShadowDuskEffectProcessor` |
...
| `SD0600` | Slang frontend: a **Slang-only language feature** (`import`, `module`, `extension`, `associatedtype`, `interface`, `__generic`, or a generic type-parameter constraint such as `<T : IFoo>` — named in the message) appeared in the source. ShadowDusk compiles the **HLSL-compatible subset of Slang**: the shader body goes through the same pipeline as every `.fx`, with nothing extra to install on any platform (the deliberate trade, owner direction 2026-08-13 — no Slang toolchain is shipped or invoked, so features with no HLSL meaning have nothing to lower to). Rewrite the construct in plain HLSL terms — or add the `ShadowDusk.Slang` package (Phase 66), which compiles real slangc-backed Slang instead of this HLSL-compatible subset. Subtler Slang-isms this courtesy scan misses fall through to DXC, whose own verbatim diagnostics apply. | `SlangFrontend` |
...
| `SD0624` | Real-slangc Slang route: the source calls an HLSL Shader Model 6 wave/quad intrinsic (`WaveActiveSum`, `QuadReadAcrossX`, …, named in the message) targeting OpenGL, DirectX (DX11), or FNA — all three are architecturally capped below SM6 (OpenGL's fixed DXC SM5 profile, DX11's SM5 DXBC container, FNA's SM≤3 `fx_2_0`) and can never represent it. Real slangc accepts the construct; it has nowhere to land on this target. Build for Vulkan or DirectX12 instead. | `SlangCompiler`, `SlangSm6ConstructGuard` |
