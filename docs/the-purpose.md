...
| DX12 | ✅ **rung-4 proven** (Phase 54, 2026-07-23) — real MonoGame `WindowsDX12`, maxd 0 against a real `mgfxc` `DirectX_12` golden for both the PS/SpriteBatch corpus and Apos.Shapes/VS-driven | untested on this host, no reason to expect divergence (DXC compile, not vkd3d) | untested on this host, no reason to expect divergence | not an export target (no DX12 in a browser) |
