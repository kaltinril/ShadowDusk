...
5. **Direct `.xnb` output** ([Phase 60](../plan/DONE/PHASE-60-xnb-content-output.md), [issue #199](https://github.com/kaltinril/ShadowDusk/issues/199)) — ShadowDusk writes the **content-pipeline `.xnb` itself**, so the consumer drops the file where their `mgfxc`-built one sat and keeps calling `Content.Load<Effect>("Foo")` **with no code change and MGCB out of the picture entirely**. `CompiledShader.ToXnb()` on the library, or simply an `.xnb` output path on the CLI. This is the *purest* form of the drop-in promise — the only route that leaves the consumer's source tree completely untouched — and it is a container only: the payload is the same rung-4-proven `.mgfx`/`.fxb` bytes, so no shader-compilation behaviour changes. Rung-4 proven (`validation/XnbContentLoad`: a real `ContentManager.Load<Effect>` rendering pixel-identical to the `mgfxc` build). Additive — the MGCB plugin stays, for teams who *want* MGCB in their build.
...
- **"Same `.mgfx` output" means behaviorally equivalent and `Effect`-loadable — NOT byte-identical to `mgfxc`.** Different compilers; byte-equality with `mgfxc` is neither expected nor a goal. "Byte-identical / deterministic" (Core Design Constraint 3) refers only to *ShadowDusk's own* reproducibility: same ShadowDusk version + same source + same target → same bytes. **One carve-out: `DirectX12`.** DXIL validation/signing runs through the Windows-only `dxil.dll`, so a DX12 compile on Linux/macOS emits *unsigned* DXIL that retail D3D12 rejects at pipeline-state creation (warned as `SD0214`). DX12 output is therefore **host-dependent** until cross-platform signing ships; the byte-identity manifest covers `DirectX_Vkd3d`, `FNA`, and `OpenGL` only.
...
| DirectX 12 *(Phase 54 — **rung 4 proven** for MonoGame `WindowsDX12`, PS/SpriteBatch + VS-driven/Apos.Shapes corpora; KNI ships no DX12 platform)* | HLSL | DXC → SM6 DXIL (direct, no `-spirv`) |
...
| Vulkan SPIR-V | ✅ **rung-4 proven** (Phase 32, 2026-07-18) — real MonoGame `DesktopVK`, **plus a pixel-diff vs the `mgfxc` 3.8.5 golden at maxd 0** on the VS-driven and `Apos.Shapes` arms (issue #145, 2026-07-22). mgfxc's own Vulkan output still crashes on load for auto-numbered resources (a separate MonoGame bug, see `plan/DONE/PHASE-32-appendix/`), so the Phase 32 PS corpus stays own-output-proven; explicit registers keep mgfxc's slot arithmetic in range and unlock the diff | untested on this host, no reason to expect divergence (same DXC→SPIR-V frontend as OpenGL) | untested on this host, no reason to expect divergence | not an export target yet — no Vulkan-capable browser consumer |
| DX12 | ✅ **rung-4 proven** (Phase 54, 2026-07-23) — real MonoGame `WindowsDX12`, maxd 0 against a real `mgfxc` `DirectX_12` golden for both the PS/SpriteBatch corpus and Apos.Shapes/VS-driven | ⚠️ **compiles, but the output is unusable on retail D3D12** — `dxil.dll` validation/signing is Windows-only, so the DXIL is *unsigned* and rejected at pipeline-state creation (warned as `SD0214`). **Compile DX12 on Windows** until cross-platform signing ships | ⚠️ **same as Linux, and worse** — macOS ships no `dxil` at all; unsigned output, `SD0214` | not an export target (no DX12 in a browser) |
