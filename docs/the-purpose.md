...
| FNA *(Phase 39 — rung 2; one `.fxb` serves all FNA backends)* | D3D9-style HLSL (SM ≤ 3) | vkd3d-shader → D3D9 bytecode → ShadowDusk `Fx2EffectWriter` → fx_2_0 (`.fxb`) |
...
> real FNA renders pixel-equivalent. **Rungs 1–2 are proven; 3–4 are open — no public claim of
> FNA support until rung 4.** `fxc`/`d3dcompiler_47` are test oracles only and never ship —
> the shipping path is vkd3d-shader's SM1–3 backend on every host. This is additive reach
> (Part 1); the mgfxc-replacement promise remains the primary product.
