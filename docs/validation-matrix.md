...
| **Android** (on-device compile) | ⬛ **planned (Phase 50)** — no `android-*` RID / NDK natives yet | ⬛ none yet. **No hard blocker found** (in-process P/Invoke, no child-process spawn, no temp-file I/O); needs NDK-built DXC + SPIRV-Cross, an APK-`lib/<abi>/` W^X-safe loader branch, and a device/emulator render rung. **Build-time precompile-to-`.mgfx` on a host already works today** (the OS-independent bytes load on the device); on-device runtime compile is the additive-reach capability Phase 50 scopes. |
...
| **Android — compile `.fx` on-device** | **planned (Phase 50); no hard blocker, bounded native work** | Adds **Android** to the OS list so the same OpenGL/GLSL pipeline compiles `.fx` → `.mgfx` at runtime on a device (DX/FNA are desktop-only, not built for the NDK). The 2026-06-28 three-agent eval found the product library is **100% in-process P/Invoke with zero `Process.Start` and no temp-file I/O** in the compile path — the exact property Android needs (it forbids `fork`/`exec`). Remaining: **NDK cross-builds** of SPIRV-Cross (easy, officially Android-tested) and **DXC** (hard, but **de-risked by the existing `.wasm-build` DXC cross-compile** — host-tablegen split + CMake patches port to the NDK); an **Android loader branch** loading `.so` from the read-only APK `lib/<abi>/` (the desktop `SpvcLoader`/`DxcLoader` extract-to-temp path **violates Android W^X**); `android-arm64` RID + per-ABI packaging; and a **byte-identity-⇒-transitive-render** proof (Android bytes == proven desktop bytes, transferring Phase 17) plus a device/emulator load+render rung. **Build-time precompile-to-`.mgfx` on a host already works today** and is the seamless default for shipping games; on-device runtime compile is additive reach (the WASM-in-browser analogue). Until a rung-4 device proof lands, on-device Android compile would ship **experimental** (the Metal pattern). Tracked: [`plan/PHASE-50-android-runtime-support.md`](../plan/PHASE-50-android-runtime-support.md). |
