...
**Last updated:** 2026-06-18 (Phase 45 B10: the reserved-word free-uniform (`float noise;`) binding is now also render-proven through **MGFX v11** (real MonoGame 3.8.5, `validation/MonoGameV11 -- reservedword`), **KNI MGFX v10**, and **KNIFX v11** (real KNI v4.2.9001, `validation/KniDesktopGL -- reservedword [knifx]`), exact pixels grey 64/191, via the shared `ReservedWordProbe`, since the B10 fix lives in the shared GL container path). Prior: 2026-06-17 (Phase 45 B10: added the `validation/ReservedWordGl` GL render gate, reserved-word free uniform binds + drives output, wired into `validation-render.yml`); 2026-06-17 (issue #106 relational/ternary parser regression: added to the §6 compile-rung coverage); 2026-06-15 (Phase 44: KNI DirectX render-proven; GL render gates wired into CI).
