...
| **KNIFX v11** (KNI) | KNI 4.02+ | ⬛ **committed, building** | **A faithful KNIFX container writer is a planned additive output** (Phase 35 Area B) so consumers get KNIFX's XNA-compat/quality fixes (sampler-without-texture, `Matrix4x4` preservation, distinct GL vs GL-ES artifacts, full-precision GL-ES fragment ops, clearer exceptions). No writer **yet**. v10 also loads in KNI via the MGFX-v10 migration path, a **convenience, not a substitute** for emitting KNIFX. KNIFX = a new container + those parity fixes over a still-MojoShader body. See [`PHASE-35-appendix/knifx-vs-mgfx-v11-research.md`](../plan/PHASE-35-appendix/knifx-vs-mgfx-v11-research.md). |
