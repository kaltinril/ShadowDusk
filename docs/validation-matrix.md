...
| **Issue [#107](https://github.com/kaltinril/ShadowDusk/issues/107) — `do{}while(false)` rejected by WebGL** | **open (real, render/load-affecting)** | A helper with a nested `if` + early `return` (Vic's `TestEarlyReturn`) makes SPIRV-Cross emit a `do { … } while(false);` in the GL GLSL (the early `return` becomes `break`). The effect **compiles and loads on desktop GL**, but **WebGL/KNI rejects the `do`-loop at load** (xnafiddle). `mgfxc`/MojoShader doesn't emit the loop. Confirmed reproducing 2026-06-27. Distinct from #106 (CLOSED, Phase 45 parser fix). Fix likely needs a SPIRV-Cross codegen knob or a GLSL control-flow post-pass; needs a KNI WebGL load/render gate to prove. |
