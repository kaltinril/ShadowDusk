...
| **KNIFX container on KNI WebGL** | **open (additive-target refinement; empirically confirmed 2026-06-27) — NOT a KNI-web blocker** | ShadowDusk's `--target-runtime kni-knifx` emits a KNIFX (`KNIF`) container whose outer backend directory carries only the **OpenGL** backend (`0x0011`) and whose inner GL ShaderCode carries only the desktop **GLSL-1.10** entry. It **loads + renders on KNI DESKTOP GL** (`validation/KniDesktopGL knifx`, maxd 0). The browser harness was used to probe KNI **WebGL**, which **rejects it**: *"Effect profile 'DirectX_11' is not compatible with the graphics backend 'WebGL'"* — KNI WebGL reports a distinct backend (`KnifxBackend.WebGL = 0x0014`) and finds no matching directory entry. Closing it needs BOTH an outer WebGL backend entry AND inner **converted ES** GLSL entries (1.00 / 3.00es), i.e. a desktop-GLSL→ES converter ShadowDusk doesn't have yet (already flagged as a refinement in `KnifxWriter.cs`). **Crucially this is NOT a hole in KNI-web support:** the seamless **default MGFX v10** loads + renders in KNI WebGL (render-proven, the corpus + #107), so a Gum/KNI-web consumer using the default is covered today; KNIFX-on-web is an opt-in additive refinement. Tracked in [`PHASE-35-forward-version-support.md`](../plan/PHASE-35-forward-version-support.md). |
