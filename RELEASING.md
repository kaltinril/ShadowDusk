...
| `ShadowDusk.ContentPipeline` | The same importer/processor as a normal `lib/net8.0` library for MonoGame 3.8.5's Content Builder project (Phase 63). Declares a REAL `MonoGame.Framework.Content.Pipeline >= 3.8.2.1105` dependency plus `ShadowDusk.Compiler` (so the natives flow); packed in the main desktop loop with symbols. |
...
To bump for a release, change that one line (e.g. `0.18.0` → `0.19.0`), update
`CHANGELOG.md` (move `[Unreleased]` into a dated `[0.19.0]` section, leave a fresh empty
...
- **`docfx/images/pipeline-overview.svg`** — the published site embeds the **SVG**, not the
  ...
  `./tools/render-diagrams.ps1` (or `.sh`) and commit the result, or the site ships the old
  diagram. Quick check: `git log -1 --format=%cd docs/pipeline-overview.puml` vs the same for
  `docfx/images/pipeline-overview.svg`.
...
enter the `version` input (e.g. `0.19.0`, no leading `v`). On dispatch the workflow also
...
4. **pack + push** all nine `ShadowDusk.*` packages (`.nupkg` + `.snupkg` symbols) to
   ...
   `ShadowDusk.ContentPipeline` is gated the opposite way, on shipping `lib/net8.0/` and on
   **declaring** its `MonoGame.Framework.Content.Pipeline` + `ShadowDusk.Compiler` dependencies.
...
2. **The `ShadowDuskCLI` tool installs and runs:**
   ...
   ```bash
   dotnet tool install -g ShadowDusk.Cli --version 0.19.0
   ShadowDuskCLI --help
   ```
...
3. **The consumer (GL) self-contained path works on a clean machine:**
   ...
   ```bash
   dotnet add package ShadowDusk.Compiler --version 0.19.0
   ```
