...
To bump for a release, change that one line (e.g. `0.6.0` → `0.7.0`), update
`CHANGELOG.md` (move `[Unreleased]` into a dated `[0.7.0]` section, leave a fresh empty
...
enter the `version` input (e.g. `0.7.0`, no leading `v`). On dispatch the workflow also
...
disagree, the workflow fails fast and publishes nothing.** Dispatching `0.7.0` against a
`Directory.Build.props` that still says `0.6.0` is rejected — merge the version-bump PR
...
2. **The `ShadowDuskCLI` tool installs and runs:**
   ...
   ```bash
   dotnet tool install -g ShadowDusk.Cli --version 0.7.0
   ShadowDuskCLI --help
   ```
...
3. **The consumer (GL) self-contained path works on a clean machine:**
   ...
   ```bash
   dotnet add package ShadowDusk.Compiler --version 0.7.0
   ```
