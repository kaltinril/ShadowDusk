...
To bump for a release, change that one line (e.g. `0.10.0` → `0.11.0`), update
`CHANGELOG.md` (move `[Unreleased]` into a dated `[0.11.0]` section, leave a fresh empty
...
enter the `version` input (e.g. `0.11.0`, no leading `v`). On dispatch the workflow also
...
disagree, the workflow fails fast and publishes nothing.** Dispatching `0.11.0` against a
...
2. **The `ShadowDuskCLI` tool installs and runs:**
   ...
   ```bash
   dotnet tool install -g ShadowDusk.Cli --version 0.11.0
   ShadowDuskCLI --help
   ```
