...
To bump for a release, change that one line (e.g. `0.15.0` → `0.15.1`), update
`CHANGELOG.md` (move `[Unreleased]` into a dated `[0.15.1]` section, leave a fresh empty
...
enter the `version` input (e.g. `0.15.1`, no leading `v`). On dispatch the workflow also
...
2. **The `ShadowDuskCLI` tool installs and runs:**
   ...
   ```bash
   dotnet tool install -g ShadowDusk.Cli --version 0.15.1
   ShadowDuskCLI --help
   ```
...
3. **The consumer (GL) self-contained path works on a clean machine:**
   ...
   ```bash
   dotnet add package ShadowDusk.Compiler --version 0.15.1
   ```
