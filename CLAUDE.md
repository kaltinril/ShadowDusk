...
The **rung-4 render proofs** — the actual product bar (*"loads + renders like `mgfxc`/`fxc` in the real engine"*) — live in the **`validation/*` console drivers**, which are deliberately **not in `ShadowDusk.slnx` and not run by `dotnet test`**. The **OpenGL** gates run in CI on Linux (Mesa llvmpipe). **Issue #204** (2026-09-10) added a **DX-on-WARP** CI lane (`windows-latest`, no GPU, pinned to Windows' bundled software D3D rasterizer): `DxModernFeatures`, `KniWinFormsDX`, the `BaselineDx`+`CandidateDx` DX11 corpus, and the `BaselineDx12`+`CandidateDx12` DX12 corpus. Everything else — the **DX11/DX12 Apos.Shapes gallery, the ShaderToy DX route, FNA, real-KNI-desktop-GL, Vulkan, browser-ANGLE** — still has **no headless CI driver** — so **the developer's Windows box with a DX12-capable GPU is the gate.** Authoritative driver list + exact commands: [docs/validation-matrix.md](docs/validation-matrix.md) §6.
