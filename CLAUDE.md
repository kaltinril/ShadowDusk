...
**Stack:** C# 12; the shipped libraries multi-target **`net8.0` + `net10.0`** (`Cli`, `MgcbPlugin`, `ContentPipeline`, and `Wasm` stay single-TFM), so building the solution needs **both** SDKs. xUnit + Shouldly (**FluentAssertions is banned — issue #171, licence; see [project_facts.md](project_facts.md)**), warnings-as-errors. Native interop: `Vortice.Dxc` (DXC), `Silk.NET` P/Invoke (SPIRV-Cross), `vkd3d-shader` (DXBC). Ships as ten `ShadowDusk.*` NuGet packages at one shared version plus the `ShadowDuskCLI` dotnet tool.
...
The **rung-4 render proofs** — the actual product bar (*"loads + renders like `mgfxc`/`fxc` in the real engine"*) — live in the **`validation/*` console drivers**, which are deliberately **not in `ShadowDusk.slnx` and not run by `dotnet test`**. The **OpenGL** gates run in CI on Linux (Mesa llvmpipe). **Issue #204** (2026-09-10) added a **DX-on-WARP** CI lane (`windows-latest`, no GPU, pinned to Windows' bundled software D3D rasterizer): `DxModernFeatures`, `KniWinFormsDX`, and the `BaselineDx`+`CandidateDx` DX11 corpus — all three measured green in real CI. **The DX12 corpus (`BaselineDx12`+`CandidateDx12`) was also tried there and initially measured RED**: `MonoGame.Framework.Native`'s SDL2 window creation needs a Vulkan surface regardless of the WARP pin, and the runner has no GPU/Vulkan ICD at all. **Fixed same day (issue #209)** by installing Google SwiftShader (a CPU Vulkan ICD + loader) in the same job — WARP still does the D3D12 rendering, SwiftShader only satisfies SDL's window-creation probe — now measured green too. Everything else — **the DX11/DX12 Apos.Shapes gallery, the ShaderToy DX route, FNA, real-KNI-desktop-GL, Vulkan, browser-ANGLE** — still has **no headless CI driver** — so **the developer's Windows box with a DX12-capable GPU is the gate.** Authoritative driver list + exact commands: [docs/validation-matrix.md](docs/validation-matrix.md) §6.
