...
- OpenGL cannot yet emit a sampler record per (texture, sampler) pair, so several textures read through one shared `SamplerState` are rejected with `SD0216` instead of compiled; DirectX and DirectX 12 handle that shape correctly. Closing it needs the pair list plumbed out of SPIRV-Cross (`spvc_compiler_get_combined_image_samplers`).
