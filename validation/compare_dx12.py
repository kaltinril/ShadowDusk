#!/usr/bin/env python3
"""Phase 54 DirectX12 rung-4 comparison: 10 baseline (real mgfxc DirectX_12 goldens,
built by MonoGame 3.8.5's own content pipeline /Platform:WindowsDX12, loaded in real
MonoGame WindowsDX12) vs 10 candidate (ShadowDusk DirectX12 .mgfx loaded in the SAME
WindowsDX12 Effect path).

Same-backend only: ShadowDusk-DX12 vs mgfxc-DX12, both rendered by real WindowsDX12.
For each shader, reports whether both images exist, the number of differing pixels,
the max per-channel delta, and the mean delta. Exit code is non-zero if any pair is
missing or differs beyond the tolerance.

Usage:  python compare_dx12.py [--tolerance N]
...
BASELINE = os.path.join(HERE, "output", "baseline-dx12")
CANDIDATE = os.path.join(HERE, "output", "candidate-dx12")
DIFF_DIR = os.path.join(HERE, "output", "diff-dx12")
