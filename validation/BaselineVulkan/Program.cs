// Baseline = the OFFICIAL mgfxc 3.8.5 Vulkan output. Loads the checked-in
// tests/fixtures/golden/Vulkan/*.mgfx (real dotnet-mgfxc 3.8.5 /Profile:Vulkan output)
// into a real DesktopVK Effect, applies each to the cat, saves PNGs.
...
string goldenDir = Path.Combine(repoRoot, "tests", "fixtures", "golden", "Vulkan");
string catPath = ShaderInputs.CatPath(repoRoot);
string outDir = Path.Combine(repoRoot, "validation", "output", "baseline-vulkan");
...
Console.WriteLine($"[baseline-vulkan] cat: {catPath}");
Console.WriteLine($"[baseline-vulkan] goldens: {goldenDir}");
Console.WriteLine($"[baseline-vulkan] out: {outDir}\n");
...
Console.WriteLine($"\n[baseline-vulkan] {ok}/{game.Outcomes.Count} rendered.");
