...
// in-memory (Vulkan target), loads the resulting .mgfx bytes into the SAME real
// DesktopVK Effect path as the baseline, applies each to the cat, saves PNGs.
...
string outDir = Path.Combine(repoRoot, "validation", "output", "candidate-vulkan");

Console.WriteLine($"[candidate-vulkan] cat: {catPath}");
Console.WriteLine($"[candidate-vulkan] shaders: {shaderDir}");
Console.WriteLine($"[candidate-vulkan] out: {outDir}\n");
...
foreach (var name in ShaderInputs.ShaderNames)
{
    ...
    {
        string mgfxDir = Path.Combine(outDir, "..", "candidate-vulkan-mgfx");
...
Console.WriteLine("[candidate-vulkan] compile results:");
...
int ok = 0;
Console.WriteLine("[candidate-vulkan] load + render results:");
...
Console.WriteLine($"\n[candidate-vulkan] {ok}/{game.Outcomes.Count} rendered.");
