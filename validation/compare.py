#!/usr/bin/env python3
"""Compare the 20 validation images: 10 baseline (pure MonoGame / mgfxc goldens)
vs 10 candidate (ShadowDusk compiler -> MonoGame).

For each shader, reports whether both images exist, the number of differing
pixels, the max per-channel delta, and the mean delta. Exit code is non-zero
if any pair is missing or differs beyond the tolerance.

Usage:  python compare.py [--tolerance N]
...
DIFF_DIR = os.path.join(HERE, "output", "diff")
...
def main():
    ...
    for name in SHADERS:
        ...
        if diff_px != 0:
            failures += 1
            # magenta where over tolerance, else the baseline pixel
