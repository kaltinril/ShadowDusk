...
- **New project `validation/ForwardCompat/`** (mirrors `validation/Candidate/`): compiles the SM3 PS-only corpus with the **unchanged** `EffectCompiler` (default opts → **v10 GL `.mgfx`**) and renders those exact bytes in a **real `MonoGame.Framework.DesktopGL` 3.8.4.1 `Effect`**. The newer runtime is pulled in **only** here via `<PackageReference … VersionOverride="3.8.4.1" />` (rationale documented in the csproj) — product pin untouched, project not in `ShadowDusk.slnx`, never packed. The harness prints the loaded MonoGame `AssemblyInformationalVersion` as proof it ran on 3.8.4.1.
- **Forward-compat matrix** (full table in `validation/ForwardCompat/README.md`): **10/10** compile + load + render on 3.8.4.1; **pixel-identical (maxΔ 0)** to the 3.8.2.1105 renders of the same bytes; **≤ tolerance** vs the mgfxc goldens (maxΔ ≤ 1, same as the original Phase 17 fidelity result).
- **Regression guard:** `validation/ForwardCompat/run-forwardcompat.ps1` (renders the three sets + pixel-compares, exits non-zero on any divergence) backed by `validation/compare_forwardcompat.py`. Re-run instructions in the README.
