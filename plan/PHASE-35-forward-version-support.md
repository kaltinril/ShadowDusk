...
**Area B follow-on — KNIFX does NOT load on KNI WebGL (empirically confirmed 2026-06-27; additive-target refinement, NOT a KNI-web blocker):** while building a KNI WebGL render proof for the `__KNIFX__` branch, the browser harness empirically demonstrated that ShadowDusk's `--target-runtime kni-knifx` output **loads + renders on KNI DESKTOP GL but is REJECTED by KNI WebGL**: `new Effect(gd, bytes)` throws *"Effect profile 'DirectX_11' is not compatible with the graphics backend 'WebGL'"*. Root cause is the two-layer GL directory in `KnifxWriter`: (1) the **outer KNIFX backend directory** carries only the OpenGL backend (`KnifxBackend.OpenGL = 0x0011`), while KNI WebGL reports `KnifxBackend.WebGL = 0x0014` and finds no match; and (2) the **inner GL ShaderCode version directory** carries only the desktop **GLSL-1.10** entry (`ES=false`), whereas WebGL needs converted **ES** entries (1.00 / 3.00es). Closing it needs both directory entries added AND a desktop-GLSL→ES converter ShadowDusk does not have (the MGFX path leans on KNI's *runtime* converter instead; whether that converter also runs for a matched KNIFX WebGL body is the open question to test first). This was already flagged as a "future refinement" in `KnifxWriter.BuildGlShaderCode`; the comment there now carries the empirical evidence. **Why this is not a KNI-web blocker:** the seamless **default MGFX v10** loads + renders in real KNI WebGL (render-proven: the 10 PS-only corpus + the #107 do-while repro, Reach + HiDef, via `tests/ShadowDusk.BrowserTests`), so a Gum/KNI-web consumer using ShadowDusk's default is covered today. KNIFX-on-web is an opt-in additive refinement, not a path a consumer must take for correct output. Tracked in the validation matrix §7.
