...
**Open verification tail (the gap between "works in CI" and the doc's full fidelity bar):** (1) ✅ **DONE 2026-06-11 (PR #46)** — `CrossHostByteIdentityTests` asserts a 102-entry win-x64-generated SHA-256 manifest (`tests/fixtures/golden/byte-identity/` — OpenGL 37, DirectX-vkd3d 37, FNA 28; LF-normalized input, fixed relative `SourceFileName`, leak-guard tests) on every OS; green with 0 skips on ubuntu + macos + windows (run 27333985586, Integration 179/179 per OS) — macOS/Linux bytes proven == win-x64, regen via `SHADOWDUSK_REGENERATE_BYTE_MANIFEST=1` (see the README there); (2) the scratch-consumer check on a real Mac (`dotnet add package` → compile — needs the **next** release; 0.3.0 shipped before A/B landed); (3) rung-4 render-vs-`mgfxc` of non-Windows-compiled output (needs a GL-capable non-Windows host — llvmpipe now demonstrated by the browser smoke, so extending ImageTests there is plausible; note item (1) already transfers the Windows rung-4 results byte-for-byte — the corpus' Linux/macOS bytes ARE the render-proven Windows bytes, so (3)'s remaining value is an independent end-to-end check, not new fidelity coverage); (4) hardening ImageTests so headless soft-skips can't masquerade as passes again (the Finding-B "paradox" lesson).
