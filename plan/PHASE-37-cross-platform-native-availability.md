...
**Open verification tail (the gap between "works in CI" and the doc's full fidelity bar):** (1) a cross-host byte-identity assertion in CI (macOS/Linux `.mgfx` == win-x64 bytes — implied by determinism + the same pinned compiler source, not yet directly asserted); (2) the scratch-consumer check on a real Mac (`dotnet add package` → compile — needs the **next** release; 0.3.0 shipped before A/B landed); (3) rung-4 render-vs-`mgfxc` of non-Windows-compiled output (needs a GL-capable non-Windows host — llvmpipe now demonstrated by the browser smoke, so extending ImageTests there is plausible); (4) hardening ImageTests so headless soft-skips can't masquerade as passes again (the Finding-B "paradox" lesson).
