...
- [ ] **Verify the per-shader record's attribute-table field for the DX profile.** Phase 17's writer always emits the GL vertex-attribute table (count byte + entries; 0 for PS-only). Byte-walk a `DirectX_11/` golden against MonoGame's DX `Shader` reader to confirm whether the DX record contains/omits that table — if DX omits it, make the writer profile-aware so a DX `.mgfx` doesn't desync (the GL path is unaffected). This is the DX analogue of the §3.1 byte-decode that caught the GL cbuffer interleave bug.
