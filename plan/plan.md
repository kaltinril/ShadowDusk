...
| 37 | 🟠 Open — **C ✅ done (PRs #35–#37, 2026-06-10)**; A + B remain | [PHASE-37-cross-platform-native-availability.md](PHASE-37-cross-platform-native-availability.md) | **Cross-platform integration-test native availability** — the `ci.yml` Integration job is red for **three distinct reasons**: **(A, product gap)** `Vortice.Dxc` ships **no macOS native** → `DllNotFoundException` on every Mac compile (consumer-facing, not CI-only) → fix = build our own `libdxcompiler.dylib` @ pinned `e043f4a1` + a `DllImportResolver` on the Vortice assembly; **(B)** the Linux DXC ICE (absorbs Phase 36; root cause unresolved — same-run paradox); **(C ✅ done)** all four pinned vkd3d 1.17 per-RID binaries are hosted on the fixed release tag `native-vkd3d-1.17`, downloaded + SHA-256-verified by `tools/restore.*`, cached in CI, and packed (with the LGPL notice) under a hard release gate — natives restore hash-OK on all 3 OS and the FNA compile suite now runs green on ubuntu/macOS (was silently skipping). **C residue ✅ resolved (2026-06-10, Phase 18 Track A):** DXBC reflection is now the pure-managed `RdefReader` (ShadowDusk.Core; D3DReflect demoted to a test oracle, deep-equality parity proven on both backends' DXBC, zero `.mgfx` byte change by full-corpus A/B), so the DX11 `.mgfx` pipeline runs end-to-end on any OS via the vkd3d backend — and DXC is now constructed lazily, so DX11 compiles no longer require the DXC native at all (macOS DX11 compile unblocked ahead of A). Full issue/cause/why/fix/test-steps + the Track A as-built section in the doc. |
