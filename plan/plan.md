...
| 34 | 🟡 Planned | [PHASE-34-gl-texture-breadth.md](PHASE-34-gl-texture-breadth.md) | **GL texture breadth** — lift the Phase 33 `SD0210` guards by actually supporting the GL texture features the MojoShader rewriter didn't model. **Cube maps** (`TextureCube`): full support on Desktop + Reach + HiDef. **3D textures + LOD/grad sampling** (`Texture3D`, `SampleLevel`/`SampleGrad`): supported on Desktop + HiDef/WebGL2; **not on Reach/WebGL1** — the platform itself lacks them (3D is WebGL2-only; fragment LOD is extension-gated), the one part we *can't* fix and must clearly diagnose instead. *(Fidelity/reach track; deferred from Phase 33.)* |
