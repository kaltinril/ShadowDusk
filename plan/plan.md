...
| 22 | In progress | [PHASE-22-wasm-shader-fiddle-sample.md](PHASE-22-wasm-shader-fiddle-sample.md) | KNI Blazor-WASM **sample app** (`samples/ShaderFiddle.Web/`, net8.0-browser): paste `.fx` → compile in-browser via `ShadowDusk.Wasm` → `new Effect` → cat + shader + error UI. **Built & build/publish-verified (2026-05-31)**; mode-1 load+render of the 10 Phase-17 goldens wired. Mode-2 in-browser compile is now **implemented for real** (Slang-wasm frontend + SPIRV-Cross-wasm, via Phase 23), each stage node-verified. ⚠️ The Slang frontend is a **sample-only** substitute — **not** the faithful DXC pipeline (a known drift from THE PURPOSE); faithful DXC→WASM for the *product* stays open (Phase 23). Pending a browser run: the rendered-cat bar + the KNI MGFX-v10-vs-KNIFX-v11 load-parity question |
| 23 | Active | [PHASE-23-in-browser-compilation.md](PHASE-23-in-browser-compilation.md) | **In-browser mode-2 compile**, un-deferred from Phase 100. Split seam: **SPIRV-Cross → WASM (built, node-verified byte-identical to desktop)** — reusable by the faithful path; **DXC → WASM** is the hard part (its LLVM fork). The sample currently uses **Slang-wasm** as the HLSL→SPIR-V frontend (cheap, built, node-verified) — but per THE PURPOSE that is **sample-only**; the **faithful** product path is DXC→WASM (multi-day, fidelity-safe) and **remains open**. Emscripten pinned to **3.1.34** (the .NET 8 runtime's). DoD: a corpus shader compiles fully in-browser via the *faithful* pipeline, bytes/behavior matching the CLI |
...
| 100 | Deferred | [PHASE-100-deferred-backlog.md](PHASE-100-deferred-backlog.md) | **Single deferred bucket** (merges the old Phase 20 backlog + leftover test/verification items), parked far-future: unchecked items from phases 2–9. **The WASM browser-runtime tail (emscripten DXC + SPIRV-Cross modules + in-browser Mode 1/2 run) was promoted out to the active [Phase 23](PHASE-23-in-browser-compilation.md) on 2026-05-31.** No prerequisites; review before 1.0 |
