...
| 37 | 🔴 Open (new 2026-06-07) | [PHASE-37-cross-platform-native-availability.md](PHASE-37-cross-platform-native-availability.md) | **Cross-platform integration-test native availability** — the `ci.yml` Integration job is red for **three distinct reasons**: **(A, product gap)** `Vortice.Dxc` ships **no macOS native** → `DllNotFoundException` on every Mac compile (consumer-facing, not CI-only) → fix = build our own `libdxcompiler.dylib` @ pinned `e043f4a1` + a `DllImportResolver` on the Vortice assembly; **(B)** the Linux DXC ICE (absorbs Phase 36; root cause unresolved — same-run paradox); **(C)** the opt-in `vkd3d` native is absent in CI → host pinned 1.17 per-RID binaries + `restore` download. Full issue/cause/why/fix/test-steps/investigation-technique in the doc. |
...
- **Backend breadth (post-1.0, validation-gated):** [31](PHASE-31-metal-msl-backend.md) Metal/MSL · [32](PHASE-32-vulkan-backend.md) Vulkan (parked) · [39](DONE/PHASE-39-fna-fx2-output-target.md) ✅ FNA fx_2_0 (all four rungs proven, PS-only + VS-driven — real FNA renders pixel-identical to fxc; osx binaries remain) · [4.1](PHASE-4.1-SPIKE-wasm-directx-dxbc.md) DirectX-DXBC-in-WASM spike (parked) — each blocked on a real runtime to render-validate against.
...
- **Browser = export station (owner-directed, 2026-06-09):** compile-target ≠
  ...
  WASM. Completion order: 37 C (+ macOS vkd3d) → 37 A/B → 4.1 → Metal/Vulkan stay
  validation-gated.
