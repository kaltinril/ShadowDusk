...
| 39 | ✅ Done — rung 4 (2026-06-09) | [DONE/PHASE-39-fna-fx2-output-target.md](DONE/PHASE-39-fna-fx2-output-target.md) | **FNA fx_2_0 output target** — `PlatformTarget.Fna` compiles D3D9-style `.fx` → the legacy D3D9 Effects binary (`0xFEFF0901`, `.fxb`) FNA loads: vkd3d `D3D_BYTECODE` SM1–3 blobs (pinned 1.17) + new `Fx2EffectWriter`/`Fx2EffectBuilder`/`CtabReader`/`D3d9BytecodePatcher` per the MojoShader-derived spec `docs/fx2-binary-format.md`. **All four evidence rungs proven**: structural validation calibrated against `fxc /T fx_2_0` goldens, and `validation/FnaValidation` renders the corpus in **real FNA 26.06 pixel-identical to the fxc oracle (gate 10/10 + 12 extended, Δ ≤ 1/255)** — after fixing three vkd3d-vs-MojoShader codegen incompatibilities (texkill writemask, pre-SM3 texld swizzle, the ±2³² def-literal LLP64 print bug). **Self-contained NuGet proven for win-x64 + linux-x64** (`runtimes/<rid>/native` packing + loader probe; cross-host SHA256-identical output). Purely additive (existing GL/DX bytes verified unchanged vs main). Follow-ups: VS-driven FNA effects, osx binaries, CI artifact hosting (37 C). |
...
- **Backend breadth (post-1.0, validation-gated):** [31](PHASE-31-metal-msl-backend.md) Metal/MSL · [32](PHASE-32-vulkan-backend.md) Vulkan (parked) · [39](DONE/PHASE-39-fna-fx2-output-target.md) ✅ FNA fx_2_0 (all four rungs proven — real FNA renders pixel-identical to fxc; VS-driven FNA effects + osx binaries remain) · [4.1](PHASE-4.1-SPIKE-wasm-directx-dxbc.md) DirectX-DXBC-in-WASM spike (parked) — each blocked on a real runtime to render-validate against.
