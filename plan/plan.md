...
| 35 | 📋 Shell (not started) | [PHASE-35-forward-version-support.md](PHASE-35-forward-version-support.md) | **Forward-version support & validation (opt-in)** — support newer versions *without* breaking backwards compat (**default stays MGFX v10 / MonoGame 3.8.2**; everything additive/opt-in or validation-only). Scaffold for future agents. Areas: **(A)** forward-compat validation of v10 output on MonoGame 3.8.4.1/3.8.5 *(doable now)* · **(B)** opt-in newer MGFX v11/KNIFX · **(C)** DX12/DXIL render-validation *(gated on 3.8.5 stable)* · **(D)** un-park Vulkan (Phase 32) *(gated on 3.8.5 stable)*. *(Forward-compatibility track.)* |
...
- **Forward-compatibility (newer versions, opt-in):** [35](PHASE-35-forward-version-support.md) forward-version support — validate v10 output on newer MonoGame (3.8.4.1/3.8.5); opt-in MGFX v11/KNIFX, DX12/DXIL render-validation, Vulkan un-park (the last two 3.8.5-gated). **Additive only — v10 / MonoGame 3.8.2 stays the default** (backwards compat is a hard requirement).
