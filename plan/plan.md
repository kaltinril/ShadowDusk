...
| 30 | Planned | [PHASE-30-ci-and-nuget-release.md](PHASE-30-ci-and-nuget-release.md) | **CI + NuGet release + `/release` automation.** (1) GitHub Actions CI across Linux/macOS/Windows — RID matrix (SPIRV-Cross **+ vkd3d-shader**), native restore, **+ the WASM build & headless-browser smoke (§16)** that Phases 22/23/24/100 defer to it; (2) **multi-package NuGet release** (`release.yml`) — pack & push **all six `ShadowDusk.*` packages + the `mgfxc` tool** on a `v*` tag/dispatch, with a tag↔version guard; (3) a **`/release` skill (§17)** modelled on KernSmith that bumps the **centralized** version (§17.1), curates `CHANGELOG`/`RELEASING`, opens the PR, waits for CI, merges, and triggers publish |
