...
| 36 | 🟠 Open (narrow) | [PHASE-36-dxc-linux-spirv-ice.md](PHASE-36-dxc-linux-spirv-ice.md) | **DXC `Internal Compiler error` on the Linux Debug-CLI / spawned-process compile path** (the `wasm.yml` browser-smoke red badge). **Scope corrected:** the real ubuntu runner DOES compile via DXC (ImageTests 27/27 incl. `ImageRegressionTests` which compile via `EffectCompiler` for OpenGL) — so desktop in-process reach is intact. The failure is the `compile-corpus-sd.mjs` Debug-CLI/spawned path (WSL repro was a degraded env, unreliable). Likely-quick experiment: build that CLI `-c Release`. **Does NOT block the WASM/KNI publish** (browser uses dxcompiler.wasm) or consumers (output render-validated on Windows). |
