...
| 23 | Active | [PHASE-23-in-browser-compilation.md](PHASE-23-in-browser-compilation.md) | **Faithful in-browser mode-2 compile** for the *product*. **DECIDED (2026-06-01): Option A — faithful DXC→WASM is the only product frontend; Slang is sample-only** (no substitute compilers). SPIRV-Cross→WASM is built & node-verified byte-identical and ships as a `[JSImport]` static web asset; the **DXC→WASM build is the long pole** (recipe now in the doc). Emscripten pinned to **3.1.34**. DoD: a corpus shader compiles fully in-browser via the faithful pipeline, bytes identical to the CLI, render-proven on Phase 24's harness |
| 24 | Active (next up) | [PHASE-24-browser-render-validation.md](PHASE-24-browser-render-validation.md) | **Browser render validation (Playwright headless)** — the first real-browser run of ShadowDusk output. Mode-1 first: do the 10 corpus `.mgfx` load + render pixel-equivalent in **real KNI WebGL** (resolves the unverified MGFXReader10/KNIFX-v11 question)? Then mode-2. **Cheapest, highest-info step; gates Phase 23's render proof** — run before the DXC→WASM build effort. Owns the harness Phase 30 CI runs |
