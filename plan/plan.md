...
| 4.1 | 🟡 Un-parked (2026-06-09) | [PHASE-4.1-SPIKE-wasm-directx-dxbc.md](PHASE-4.1-SPIKE-wasm-directx-dxbc.md) | **vkd3d-shader → WASM** — elevated from far-future spike to the keystone of the owner's browser-export-station vision: ONE wasm artifact closes BOTH the DX-DXBC and FNA-fx_2_0 browser cells (their writers/patcher/reflection are managed C# that already runs in WASM; same pinned 1.17 source ⇒ no substitute compiler, byte-identity achievable). Sequence after Phase 37; reuse Phase 23's emscripten infrastructure. Server-relay stays **out of bounds** for the product (a consumer hosting the library server-side is their own legitimate choice). |
