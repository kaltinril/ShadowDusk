...
| 34 | 🟢 Implemented (branch `phase34-gl-texture-breadth`) | [PHASE-34-gl-texture-breadth.md](PHASE-34-gl-texture-breadth.md) | **GL texture breadth** — lifted the Phase 33 `SD0210` guards by actually supporting the GL texture features the MojoShader rewriter didn't model. **Cube maps** (`TextureCube`): full support on Desktop + Reach + HiDef (legacy `samplerCube`/`textureCube`, sampler-`Type` byte 1; cross-validated vs the `mgfxc` cube golden). **3D textures + LOD/grad** (`Texture3D`, `SampleLevel`/`SampleGrad`): Desktop + HiDef via `texture3D` / generic `textureLod`/`textureGrad`; **Reach/WebGL1 = documented platform wall** (3D is WebGL2-only; fragment LOD is extension-gated — not compile-detectable in a single blob). The one structural change is the sampler-`Type` byte (was hardcoded 0). Rung-4 in-engine cube/3D render carried forward. 549/549 tests green. *(Fidelity/reach track; deferred from Phase 33.)* |
...
- **Fidelity / completeness:** [28](PHASE-28-vs-driven-monogame-effects.md) VS-driven effects — compile the common custom-vertex-shader case, not just PS-only · [33](DONE/PHASE-33-webgl2-es300-hidef-output.md) ✅ KNI HiDef/WebGL2 — close the `gl_FragColor` fidelity gap so one `.mgfx` works in KNI Reach *and* HiDef · [34](PHASE-34-gl-texture-breadth.md) GL texture breadth — cube maps everywhere; 3D/LOD on Desktop+HiDef (WebGL1 can't, by platform limit).
