...
| 23 | Active | [PHASE-23-in-browser-compilation.md](PHASE-23-in-browser-compilation.md) | **In-browser mode-2 compile**, un-deferred from Phase 100 (the showcase needs it). Split seam: **SPIRV-Cross → pure-C# static-link `[DllImport]` (no JS)**; **DXC → one `[JSImport]` module** (its LLVM fork isn't statically linkable). DXC fork: build pinned DXC→WASM (fidelity-safe, multi-day) vs pivot to Slang-wasm (cheap, fidelity re-proof) — decided by an in-flight Slang spike. Emscripten pinned to **3.1.34** (the .NET 8 runtime's). DoD: a corpus shader compiles fully in-browser, bytes/behavior matching the CLI |
