...
| 19 | In progress | [PHASE-19-wasm-runtime-compilation.md](PHASE-19-wasm-runtime-compilation.md) | WASM in-browser `.fx` → `.mgfx` — the browser half of *reach*. **Reach engine built & desktop-verified (2026-05-30):** injectable backend seams; a pure-managed `SpirvReflector` proven equivalent to the DXIL oracle (10/10) that removes the Windows-only reflection blocker; the GL pipeline reflects SPIR-V and emits `.mgfx` **byte-identical** to the DXIL path (10/10); `WasmShaderCompiler` composes the pipeline with `[JSImport]` DXC/SPIRV-Cross backends and **compiles for `net8.0-browser`**. **Remaining:** real emscripten DXC/SPIRV-Cross modules + an actual browser run (Mode 1 WebGL load, Mode 2 in-browser compile) — gated on a browser/toolchain not available in-session |
