...
| 39 | 📋 Shell (not started) | [PHASE-39-fxc-oracle-monogame-fidelity.md](PHASE-39-fxc-oracle-monogame-fidelity.md) | **`fxc.exe` oracle comparison & MonoGame fidelity matching** — research/characterization. The maintainer now has a local `fxc.exe` (the real DX HLSL compiler `mgfxc` uses), so this phase systematically compares ShadowDusk output vs genuine `fxc`/`mgfxc` across the **full** fixture corpus (not just the validated 10), quantifies `d3dcompiler_47`-vs-`fxc.exe` divergence, and triages fidelity gaps (incl. the known global-initializer gap). Research only — no product pin/default/pipeline change here; `fxc.exe` is a measuring stick, never a shipping dependency. *(Fidelity/completeness track.)* |
...
- **Fidelity / completeness:** [28](PHASE-28-vs-driven-monogame-effects.md) VS-driven effects — compile the common custom-vertex-shader case, not just PS-only · [33](DONE/PHASE-33-webgl2-es300-hidef-output.md) ✅ KNI HiDef/WebGL2 — close the `gl_FragColor` fidelity gap so one `.mgfx` works in KNI Reach *and* HiDef · [34](DONE/PHASE-34-gl-texture-breadth.md) ✅ GL texture breadth — cube maps everywhere; 3D/LOD on Desktop+HiDef (WebGL1 can't, by platform limit) · [39](PHASE-39-fxc-oracle-monogame-fidelity.md) `fxc.exe` oracle comparison — full-corpus fidelity matrix vs genuine `fxc`/`mgfxc` now that a local `fxc.exe` is available.
