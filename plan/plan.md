...
| 39 | 🚧 Rung 2 (2026-06-09) | [PHASE-39-fna-fx2-output-target.md](PHASE-39-fna-fx2-output-target.md) | **FNA fx_2_0 output target** — `PlatformTarget.Fna` compiles D3D9-style `.fx` → the legacy D3D9 Effects binary (`0xFEFF0901`, `.fxb`) FNA/MojoShader loads: vkd3d `D3D_BYTECODE` SM1–3 blobs (pinned 1.17 — empirically sufficient, 105/107 corpus) + new `Fx2EffectWriter`/`Fx2EffectBuilder`/`CtabReader` per the MojoShader-derived spec `docs/fx2-binary-format.md`, structurally validated against real `fxc /T fx_2_0` goldens. Purely additive (existing GL/DX bytes untouched). **Rungs 3–4 (real MojoShader parse; real FNA render) open — no public FNA-support claims until rung 4**; library NuGet shape blocked on the shared vkd3d packaging gap (Phase 37 C / Phase 18 follow-up). |
