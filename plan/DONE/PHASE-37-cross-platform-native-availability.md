...
**Status:** ✅ **DONE (2026-06-11) — all three findings AND the verification tail closed.** Created 2026-06-07. The tail's three new gates are now standing CI: the cross-host byte-identity manifest (PR #46), the 3-OS pack→consume scratch-consumer workflow (PR #47), and hard-gated real-GL rendering on ubuntu (PR #48). **Watch items (recorded, not blocking):** one intermittent macOS `Compiler.Tests` test-host crash — native-level, no test failed, the job correctly went red (1 occurrence in ~7 macOS integration runs: run 27334063442 / job 80753809510; track frequency, suspect native-library concurrency); the Mesa `textureLod`/`textureGrad` legacy-dialect strictness (a Phase 34 follow-up — see tail item 3); the windows/macos CI lanes' GL soft-skip walls (documented in ci.yml; visible, never silent). **C ✅** (PRs #35/#36/#37 — vkd3d hosting/restore/packing). **B ✅** (PR #42 — Vortice UTF-16 args vs Linux 4-byte `wchar_t`, see Finding B). **C-residue DX11 reflection ✅** (PR #43, Phase 18 Track A — managed `RdefReader`, see the Track A section). **A ✅ functionally** (PR #41 — own pinned macOS dylibs hosted on `native-dxc-1.7.2212.40`, pins enforcing, loaded via Vortice's `Dxc.ResolveLibrary` event; macOS CI integration went ~124 `DllNotFoundException` → fully green). The Browser render smoke also passes for the first time (PR #44 — xvfb/llvmpipe + §6.1-calibrated tolerances). **`ci.yml` is green on all 3 OSes — the original "Integration job red on all three" problem is resolved.**
**Open verification tail (the gap between "works in CI" and the doc's full fidelity bar):** (1) ✅ **DONE 2026-06-11 (PR #46)** — `CrossHostByteIdentityTests` asserts a 102-entry win-x64-generated SHA-256 manifest (`tests/fixtures/golden/byte-identity/` — OpenGL 37, DirectX-vkd3d 37, FNA 28; LF-normalized input, fixed relative `SourceFileName`, leak-guard tests) on every OS; green with 0 skips on ubuntu + macos + windows (run 27333985586, Integration 179/179 per OS) — macOS/Linux bytes proven == win-x64, regen via `SHADOWDUSK_REGENERATE_BYTE_MANIFEST=1` (see the README there); (2) ✅ **DONE 2026-06-11 (PR #47)** — `.github/workflows/pack-consume.yml` packs `ShadowDusk.{Core,HLSL,GLSL,Compiler}` into a local feed, asserts the pack gate inline (all four vkd3d natives + both DXC dylibs + notices — the 0.2.0-empty-package regression net), then a scratch console app **outside the repo** does `dotnet add package ShadowDusk.Compiler` from that feed and compiles real fixtures for OpenGL + FNA — green on all 3 OSes, **the macos lane being the real-Mac scratch-consumer check** (and stronger than the original ask: it tests HEAD, not a published package; the post-publish nuget.org verification remains the RELEASING.md "Verify after release" step); (3) ✅ **DONE 2026-06-11 (PR #48)** — the Build & Test **ubuntu lane now renders the GL ImageTests corpus for real** under xvfb + Mesa llvmpipe (mirrors PR #44's recipe; `glxinfo` on the runner: **4.5 Compatibility Profile, Mesa 25.2.8** — comfortably covers the fixture's 3.3 Compatibility request). 23 tests genuinely render per run (12 golden-image rows + 9 mgfxc cross-validation rows + 2 infra facts; Dissolve still visibly skips on its pre-existing compile gap) — ImageTests 41/41 with the item-(4) hard gate ON, so "green" now PROVES rendering. One tolerance extended with an observed value per the §6.1 ladder (`multipass_pass1` 0→1: PS alpha 0.5 sits on the 8-bit quantization boundary — golden GPU stored 128, llvmpipe rounds 127; delta exactly 1, alpha-only, structure unchanged) and one **documented structural exclusion** (Phase34LodGradRenderTests off the Linux lane: Mesa's strict GLSL front-end rejects `textureLod`/`textureGrad` in the implicit-#version-110 legacy dialect — "no function with name 'textureLod'" — where NVIDIA permissively accepts them; NOT masked with tolerance. **Phase 34 follow-up:** the same emitted GLSL may fail to load in real MonoGame on Mesa desktops — worth a rung-4 check there). Caveat kept honest: this is still the **proxy** renderer, not real MonoGame `Effect` — but per item (1), the Linux bytes ARE the render-proven Windows bytes, so the remaining gap is an independent end-to-end check, not fidelity coverage; (4) ✅ **DONE 2026-06-11 (PR #48)** — **`SHADOWDUSK_REQUIRE_GL`** (pure gate logic in `GlRequirement`, unit-tested): when set, a failed GL-context creation **throws from `GlContextFixture.InitializeAsync`** — every test in the class fails loudly (the ubuntu lane sets it, so a regression back to headless-skip turns the lane red instead of fabricating 27-greens-in-73-ms); when unset, the soft-skip remains but emits an unmistakable `GL SOFT-SKIP (rendered 0)` stderr line per class + a standard `SOFT-SKIP (rendered 0 — PASS without rendering)` test-output line. Three latent maskers found and fixed en route: (a) per-class `IClassFixture` + default xUnit parallelism **raced GLFW's Win32 window-class registration** — classes could lose the race and silently soft-skip even on a GPU-equipped Windows host (now one shared `GlContextCollection` collection fixture; local Windows 51/51 with the gate on); (b) on ubuntu runners the testhost under `dotnet test <slnx> --no-build` misses the test project's RID native dirs, so Silk.NET could never load `libglfw.so.3` **by name** while an absolute-path dlopen of the same pristine ELF succeeds (probe-proven in CI; fixed by an absolute-path preload à la `SpvcLoader`) — this, not the missing display, was why ubuntu never got a GL context; (c) Silk.NET swallows the real loader error — the fixture now appends the true dlopen/resolution detail to the failure reason. **Lane truth table:** ubuntu = renders (hard-gated); **windows-latest CI = soft-skips visibly** (runners expose GDI Generic OpenGL 1.1 only — verified run 27331878150: 35 ImageTests in 604 ms; real-GPU Windows coverage stays the local dev run); **macOS = soft-skips visibly by platform design** (Apple caps native GL Compatibility at 2.1; GLFW can't exit cleanly headless — the remaining platform wall, see `GlContextFixture`).
**Track:** Reach (Part 1 of THE PURPOSE) — "compile where `mgfxc` can't (Linux/macOS)." The integration tests were RED because they were faithfully catching real cross-platform gaps; this was not a test-logic problem.
