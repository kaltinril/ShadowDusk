...
lexer's undefined token, which yields the unique `E5000 syntax error, unexpected invalid token`
— or `unexpected $undefined` from the bison-3.5 linux-x64 build, see the Linux follow-up below —
...
**Linux follow-up — the sentinel's diagnostic is spelled by bison, not by vkd3d.** The first
cut recognised the sentinel by the text `syntax error, unexpected invalid token`, measured on
the win-x64 native, and passed on Windows and macOS; on CI's ubuntu lane every `Issue202` test
came back with vkd3d's raw coordinates (`71` for user line 13, `3607` for the reporter's file)
after 128 probes that all read as "nothing fired", and the reporter's file took 47-66 s instead
of 3 s. `strings` on the four natives showed why: the linux-x64 `.so.1` contains no
`"invalid token"` at all, only `$undefined`. The name of the undefined token is generated by
bison into `hlsl.tab.c` — `"invalid token"` from bison 3.6 and newer (the MinGW win-x64 build
and both Homebrew macOS builds), `$undefined` from anything older — and the linux-x64 native is
deliberately built in an Ubuntu 20.04 container (bison 3.5.1) for its glibc 2.31 baseline, so
its sentinel diagnostic is `E5000 syntax error, unexpected $undefined`. `Vkd3dSourceLocator`
now recognises both spellings (`SentinelMessage`, `SentinelMessageLegacyBison`); the message
itself still reaches the caller verbatim in whichever form the host's native produced. Pinned
by a pure locator test with the `$undefined` fake and by a `[Vkd3dFact]` that compiles an
author's own `@` on every host and asserts the spelling is one of the two and the location is
right. Any future native rebuild on a different bison changes nothing; a third spelling would
surface as that test failing, never as silent raw coordinates.
