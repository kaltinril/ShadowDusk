...
- **Forward-compat extends to the 3.8.5 preview too (source-verified, not yet render-run):** the 3.8.5 line bumps the runtime to `MGFXVersion = 11` **but** adds `MGFXMinVersion = 10`, so its `Effect` loader accepts the **range [10, 11]** — our **v10 output is accepted by 3.8.5 unchanged** (verified against `v3.8.5-preview.2` and `develop` tags of `MonoGame.Framework/Graphics/Effect/Effect.cs`). 3.8.4.1 keeps the strict `MGFXVersion = 10` exact-match (no `MGFXMinVersion`), which our v10 output satisfies. Net: **staying on v10 is forward-safe across 3.8.2 → 3.8.4.1 → 3.8.5-preview** — so v10 is a safe *default* forever. This forward-safety means Area B is **not needed for loadability**, but Area B is still a **committed deliverable** for a different reason: to let consumers *use* the v11/KNIFX container features (it is additive, not a fallback for a broken v10). A render-run against 3.8.5 is deferred until it goes stable (Areas C/D gating). **Caveat (2026-06-12): the `[10, 11]` range here is MonoGame-specific. KNI does NOT accept a range under one signature — it accepts `MGFX`@10 OR `KNIFX`@11 (two distinct signatures); v10 stays forward-safe on KNI via its dedicated MGFX-v10 migration path, and ShadowDusk's v10 output **is now render-validated against KNI v4.02 specifically** (2026-06-14, `validation/KniDesktopGL`: real KNI `Effect` v4.2.9001 on SDL2.GL desktop, 10/10 render, maxd 0 vs MonoGame and <= 1 vs the mgfxc goldens). Full detail + loader evidence: [appendix research](PHASE-35-appendix/knifx-vs-mgfx-v11-research.md).**

### B. Newer format (MonoGame `MGFX` v11 AND KNI KNIFX) — **committed additive deliverable (build both), seamlessly**

> **Direction (2026-06-14, user):** This is **not** "only if a runtime requires it." ShadowDusk **will emit
> v11 and KNIFX** as first-class additive outputs so consumers can *use* the newer containers' features, not
> merely rely on v10 staying loadable. v10 forward-compatibility is a convenience, **not** a substitute for
> building these. The seamless rule is preserved by keeping **v10 the default** and making v11/KNIFX opt-in or
> auto-selected from the target (never a required flag). Build both faithful writers; validate each against
> its real runtime (KNI v4.02 for KNIFX, MonoGame 3.8.5 for MGFX v11).
...
ShadowDusk **auto-produces the right format** from the target — the consumer never has to pick v10 vs v11 vs KNIFX to get correct output. v10 keeps **loading** forward on both forks (Area A), so loadability alone never *forces* a newer format — but loadability is **not** the point here. The point is to let consumers **use** the newer containers' *render-quality / XNA-compat features* (§4 of the appendix), and optionally take those bytes for their own runtime. That is a committed, first-class deliverable, not a contingency we do "only if v10 breaks."
...
**Area B follow-on — the `__KNIFX__` compile macro (✅ 2026-06-27):** KNIFX faithfulness is not only the container bytes — KNI's effect compiler (`MojoEffectProcessor`) **unconditionally defines `__KNIFX__ = 1`** for every KNI build, and KNI's own `Macros.fxh` (the `_vs/_ps/_cb` register macros) plus real KNI shaders (**Apos.Shapes**, used by **Gum**/vchelaru: `#if __KNIFX__ → vs_4_0`) branch on it. ShadowDusk did not define it, so a KNIFX-targeted compile took the wrong branch. **Fixed:** `PlatformMacros.For(target, container)` appends `__KNIFX__` iff `Container == Knifx`, wired through `CompilationPipeline` (so `--target-runtime kni-knifx` / `CapabilityProfile.KniGL_4_02` get it). **Seamless preserved** — the default/universal MGFX output stays `__KNIFX__`-free; only an explicit KNIFX target defines it, exactly matching KNI's compiler. Byte-identity safe; pinned by `PlatformMacrosTests` + `CapabilityProfileByteIdentityTests` + `examples/ExKnifxMacro.fx`. Detail: **[`knifx-area-b-kickoff-brief.md`](PHASE-35-appendix/knifx-area-b-kickoff-brief.md) → "The `__KNIFX__` compile macro"**. Remaining rung: a real KNI render of a `__KNIFX__`-branching shader (manual; available on **KNI desktop GL** via `validation/KniDesktopGL -- reservedword knifx`, **not** on KNI WebGL — see the next bullet).

**Area B follow-on — KNIFX now loads + renders on KNI WebGL (CLOSED 2026-06-27):** while building a KNI WebGL render proof for the `__KNIFX__` branch, the browser harness first demonstrated that ShadowDusk's `--target-runtime kni-knifx` output loaded on KNI DESKTOP GL but was REJECTED by KNI WebGL (`new Effect(gd, bytes)` threw *"Effect profile 'DirectX_11' is not compatible with the graphics backend 'WebGL'"*): the KNIFX backend directory carried only the OpenGL backend (`0x0011`), while KNI WebGL reports a distinct backend (`KnifxBackend.WebGL = 0x0014`) and found no match. **What KNI actually does (researched, high-confidence):** the official KNI tool (KNIFXC) compiles `.fx` at BUILD time; for the WebGL/Blazor path the GLSL-ES dialect is handled per container — KNIFX v11 stores pre-built ES in the GL version directory, whereas legacy MGFX v10 ships ONE desktop GLSL blob that KNI's *runtime* `ConcreteShader.ConvertGLSLToGLSL300es` converts to ES at load. Crucially, with a per-shader `ShaderVersion == (0,0)` KNI treats the GL ShaderCode as **raw GLSL** and runs that same runtime converter (the v10 path), which is **render-proven to load on KNI WebGL** (the corpus + #107). **Fix:** `KnifxWriter` now emits, for a GL target, a **multi-backend directory advertising the whole GL family** (OpenGL + GLES + WebGL): the OpenGL (desktop) entry keeps the faithful version-directory body **byte-identical** (desktop render unchanged), and the GLES + WebGL entries share a body with `ShaderVersion (0,0)` + raw GLSL so KNI's runtime converts it to ES on the host. One `.knifx` therefore loads on KNI desktop GL, mobile GLES, **and** the browser. **Render-proven both hosts:** KNI desktop SDL2.GL 10/10 (`validation/KniDesktopGL knifx`), KNI WebGL loads + renders the KNIFX container with the `__KNIFX__` red branch (`tests/ShadowDusk.BrowserTests --corpus=sd`, `ExKnifxMacro` 196096 red px / 0 green, Mode-1 12/12). Pinned by `KnifxWriterTests` (multi-backend dir + raw web body) + the WebGL harness probe; full suite 1940 green; DX/FNA/KNI-DX render gates unaffected (KNIFX-isolated change). *(The structurally-faithful alternative — pre-baking ES into the version directory like KNIFXC, via SPIRV-Cross ES output — was deferred: it needs KNI's exact inner-directory matching (not pinned down) and a name-preserving GLSL→ES step, for no behavioral gain over reusing KNI's own proven runtime converter. The bar is "loads + renders like the reference," which this meets.)*
