...
> - `render-states.fx` additionally hits a parser error on `DepthBufferEnable = ...` — that token is not recognized by `mgfxc`'s render-state grammar.
>
...
>
> **Adapted strategy:** bootstrap the reference PNGs from **ShadowDusk's own current compiled output**. This is an *anchor regression test*: it locks in today's pixel output and fails when a future change shifts it. It cannot independently validate that ShadowDusk's GLSL is functionally equivalent to `mgfxc`'s GLSL (there is no `mgfxc` GLSL for these fixtures to compare against). For true cross-validation, the 34 production shaders in `tests/fixtures/golden/OpenGL/` could be added later — they do have valid `mgfxc`-compiled goldens — but they are out of scope for Phase 16.
...
- [x] Reference images for all 9 fixtures are checked into `tests/fixtures/reference-images/OpenGL/` (12 PNGs — 9 fixtures, 5 of which contribute extra scenes for multi-pass/multi-technique cases)
- [x] All ImageRegression theory rows pass when running ShadowDusk-compiled output against the reference images (12/12)
- [x] `SHADOWDUSK_UPDATE_GOLDEN=1` regenerates reference images (from ShadowDusk's own current output — see Section 6.1 deviation note)
...
- [x] 2. Implement `GlContext/OffscreenRenderer.cs` — GLFW window (1×1 hidden), FBO 128×128, pixel readback
- [x] 3. Implement `GlContext/GlslShaderProgram.cs` — compile/link GLSL, `GlslCompileException`, attribute location binding
- [x] 4. Implement `GlslShaderExtractor.cs` — extract VS/PS GLSL from `.mgfx` blob using `MgfxBlobReader`
- [x] 5. Implement `ImageComparer.cs` — per-channel tolerance comparison, `ImageComparison` result type
- [x] 6. Implement `SceneDescriptor.cs` — per-fixture geometry, uniforms, textures, tolerance table (Section 4.2)
- [x] 7. Implement `Rendering/ShaderSceneRenderer.cs` — compiles GLSL pair, rewrites `out_var_*` / `in_var_*` varyings to a common name so the GLSL linker matches them, uploads uniforms via UBOs (member-name lookup that handles `type_Block.Member` driver naming), renders the standard quad, returns pixel buffer.
- [x] 8. Implement `Tests/ReferenceImageGenerator.cs` — `SHADOWDUSK_UPDATE_GOLDEN=1` path. **Anchored on ShadowDusk's own current `.mgfx` output, not on `mgfxc` goldens** (see Section 6.1 deviation note).
- [x] 9. Run `ReferenceImageGenerator`; commit the 12 resulting PNGs.
- [x] 10. Implement `GlContextFixture.cs` — `IAsyncLifetime`, skip-on-no-context, thread-local context lock via `MakeContextCurrent()` guard.
- [x] 11. Implement `Tests/ImageRegressionTests.cs` — `[Theory]` over 9 fixtures × N scenes, compile → render → compare. 12 theory rows, all pass.
- [x] 12. Run `dotnet test --filter "Category=ImageRegression"` — all 13 pass (12 regression rows + 1 default-skipped generator fact).
