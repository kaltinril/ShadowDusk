...
**Carry-forwards:**
- **`roundEven` WebGL1 incompatibility → real Phase 23 prerequisite (NEW, found here).** The harness validated the **golden (mgfxc) MGFX bytes**. When the corpus is compiled by **ShadowDusk's own** OpenGL pipeline, **`Pixelated.mgfx` fails to *load* in KNI WebGL** — ShadowDusk emits `roundEven()`, which **GLSL ES 1.00 / WebGL1 does not provide** (`'roundEven': no matching overloaded function`). The golden masks this. Fix: lower `roundEven(x)` → `floor(x + 0.5)` for the GL/WebGL profile in the dialect rewrite, and extend the harness to also validate ShadowDusk's *own* output (not just the golden). This is a genuine WebGL-reach gap in the emitted GLSL.
