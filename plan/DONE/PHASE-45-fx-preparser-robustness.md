...
| **B10** | a free uniform named `noise` (a GLSL reserved word) used in a GL shader, e.g. Nez `Noise.fx` | GL compile fails `SD0012` (cbuffer/parameter join misses) | GL | **Different class — NOT a pre-parser bug.** SPIRV-Cross renames the uniform `noise` to `_noise` (GLSL reserved word) but `CompilationPipeline` joins the rewriter's uniform layout to the reflected parameter list BY NAME (`IndexOfParam`, `CompilationPipeline.cs` ~534) and the reflected list still says `noise`, so the join misses. `noise` is valid HLSL that `fxc`/`mgfxc` accept. | MED (any uniform whose name collides with a GLSL reserved word) | ✅ **Fixed** — the name join now falls back to an OFFSET BRIDGE on a `-1` result only (`IndexOfParamByRegister`): the GL uniform's `BaseRegister * 16` byte offset locates the reflected cbuffer variable, whose ORIGINAL name recovers the parameter index (single-`$Globals` case only; multi-cbuffer falls through to keep `SD0012` rather than mis-map). The `.mgfx` keeps the param under its original name. Shaders that compile today never hit `-1`, so output is byte-identical (manifest diff is +3 new fixture entries, 0 changes). Fixture `ExReservedWordUniform.fx` (GL+DX+FNA); Nez `Noise.fx` re-enabled on GL. |
