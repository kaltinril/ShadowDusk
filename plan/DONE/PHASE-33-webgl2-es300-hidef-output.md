...
**Status:** ✅ **DONE** — merged to `main` via **PR #9** (`c4d0f4d`) 2026-06-04; **issue #7 CLOSED** with the resolution comment. RED→GREEN proven in real KNI HiDef/WebGL2 (10/10 load+render), full suite **535/535** green, independently re-verified. The texture-breadth follow-up the `SD0210` guards reference was delivered in **Phase 34** (PR #10); the CI live-workflow wiring is tracked in **Phase 30 §16**. Root cause verified 2026-06-03; second review pass (consumer-invisibility + adversarial + KNI-runtime) folded in. Reframed from "add a new output target" to "close a one-spot fidelity gap."
...
| `texture3D` / `textureCube` | KNI converts the *call*, but ShadowDusk never renamed the **decl** | ✅ **DONE (loud guard).** Found pre-existing **silent breakage**: a `samplerCube`/`sampler3D` decl wasn't renamed and `texture()` was wrongly rewritten to `texture2D(non-2D sampler)` (invalid GLSL). Phase 33 made it a **loud `SD0210`**; **Phase 34 since added real support** — cube everywhere, 3D on Desktop+HiDef (decls renamed to `ps_s{k}`, dimension-specific builtin, sampler-`Type` byte). |
| `texture2DLod`/`Proj`/`Grad` | KNI does **NOT** convert | ✅ **DONE (loud guard — branch b).** FX9 `tex2Dlod/proj/grad` aren't Reach-supported (fail in DXC). Modern `SampleLevel/SampleGrad` reached the rewriter and emitted `texture2DLod`/`textureGrad` — invalid in WebGL1 FS + unconverted by KNI in WebGL2. Phase 33 made it a **loud `SD0210`**; **Phase 34 since added support** by emitting the *generic* `textureLod`/`textureGrad` (core ES 3.00; KNI passes through) on Desktop+HiDef, with the Reach limit documented. Explicit `.fx` + unit tests added. |
